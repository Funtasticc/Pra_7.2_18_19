...
struct Person	// eine Struktur mit vorname, nachname, geburtsdatum
{ // hier werden die Personendaten gespeichert als eine Art Prototyp damit man diese dann später abrufen kann oder mit den zugehörigen Funktionen bearbeiten kann.
...
Person extrahiere_person(string eingabezeile) // er nimmt die Zeichenkette oder Personendaten aus der Variablen ,,eingabezeile" welche im main() ausgelesen wurden anhand fstream
{
	...
	return p; // nun giebt er schließlich die neuangeordneten Daten in p wieder aus.
}

// Hauptprogramm
int main()
{
	string eingabezeile, // einige Stringvariablen
		kurzText, langText,
		zwichenspeicherText;
	fstream Datendatei("personendaten.txt", ios::in); // anhand von filestream nur lesen mit ios::in
	while (getline(Datendatei, eingabezeile)) // solange er die Datendatei mit enthaltenem personendaten text in eingabevariable einliest, 
	{
		Person person = extrahiere_person(eingabezeile); // wendet er die funktion extrahier.... an und speichert alles in die person und
		kurzText += "<b>" + trimme(person.nachname) + "</b>, " + person.vorname + "<br/>\n";// das (Bold) = <b></b> und (Breakline) <br/> sind HTML Befehle,
		// für Fettschrift und Zeilenumbruch also Enter.
		langText += "<b>" + trimme(person.vorname) + " " + person.nachname + "</b>, " + trimme(person.geburtsdatum) + "<br/>\n"; // greift hier auf die getrimmten 
	...
	Datendatei.close(); // Schließt die mittels fstream geöffnete Datei damit es eventuell nicht zu einem absturz kommen kann.
	...
	while (getline(Datendatei_zugriff_2, eingabezeile)) // hier wieder solange er in die Variable eingabezeile  einliest, 
	{
		for (int i = 0; i < eingabezeile.length(); i++) // zählt er wieder hoch
		{
			if (eingabezeile[i] == '%') // und prüft ob an der Stelle i im eingabe... das Zeichen % vorkommt
				zwichenspeicherText += kurzText; // falls ja Speichert er den oben erstellten kurzText(Zeile 57) und seine Werte in den zwichenspeicher
			else if (eingabezeile[i] == '$') zwichenspeicherText += langText; // falls aber das $ Zeichen vorkommt Speichert er den inhalt von langText in den zwich...
			else zwichenspeicherText += eingabezeile[i]; // ansonsten Speichert er in inhalt von eingabezeile[i] in den zwichenspeicher.
		}
		zwichenspeicherText += '\n'; // und nicht zu vergessen er soll noch ein endl in form von '\n' in der Zwichenspeicher dazuspeichern 
	}
	Datendatei_zugriff_2.close(); // nun wird wieder die Datei beendet
